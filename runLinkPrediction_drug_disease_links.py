...
def parse_args():
    ...
    parser.add_argument('--kg_covid_19_edges', type=argparse.FileType('r'),
                        help='Path to file with all edges in kg_covid_19 graph')

    parser.add_argument('--kg_covid_19_nodes', type=argparse.FileType('r'),
                        help='Path to file with all nodes in kg_covid_19 graph')

    parser.add_argument('--drug_disease_edges', type=argparse.FileType('r'),
                        help='Path to file with edges/weights for gold standard drug->disease links')

    parser.add_argument('--weight_multiplier', type=int, default=1,
                        help='Factor to multiply weight of drug/disease edges to bias random walk')
    ...
    parser.add_argument('--gamma', type=float, default=1,
                        help='hyperparameter for jumping from one network to another'
                             'network '
    ...
    parser.add_argument('--classifier', nargs='?', default='RF',
                        help="Binary classifier for link prediction, it should be "
                             "either LR, RF or SVM")

    parser.add_argument('--type', nargs='?', default='heterogen',
                        help="Type of graph which is either homogen for homogeneous "
                             "graph or heterogen for heterogeneous graph")
...
def make_train_test_files(kg_covid_19_graph,
                          drug_disease_links,
    ...
    """
    Read in drug/disease edges, split them into train/test (using test_fraction),
    then write out:
    pos_train (kg_covid_19_graph + train equivalent phenotypes)
    pos_test (test equivalent phenotypes)
    neg_train (random edges connecting nodes not connected in kg_covid_19_graph)
    neg_test (random edges connecting nodes not connected in equiv_phenotypes)

    :param kg_covid_19_graph file containing all edges from kg_covid_19 (except equivalent
    phenotypes)
    :param drug_disease_edges file containing positive drug->disease links, optionally with weights
    ...
    with open(drug_disease_links.name, 'r') as equiv_fh, \
        ...
        # append kg covid 19 graph to pos_train edges:
        with open(pos_train, 'a') as pos_train_append_fh, \
                open(kg_covid_19_graph.name, 'r') as kg_covid_19_fh:
            for line in kg_covid_19_fh:
...
def main(args):
    """
    The input files are positive training, positive test, negative training and negative
    test edges. The code reads the files and create graphs in CSFGraph format. Then, the
    positive training graph is embedded. Finally, link prediction is performed.
    ...
    if args.make_edge_files:
        ...
        make_train_test_files(kg_covid_19=args.kg_covid_19,
